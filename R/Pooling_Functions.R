...
#' Function \code{generate_distribution} generates the binomial distribution of the probability of a certain number of successes (x) in a certain number of trials (size) where the probability of success on each trial is fixed (prob).
#' Function \code{plot_cells} plots the output of both scenarios (simulation case vs. binomial distribution).
#'
#' @name pooling
...
generate_random_cells <- function(all_samples, samples_per_pool, well, outfile) {
...
#p_result <- plot_cells(samples_per_pool, well, counts, outfile)
#return(p_result)
}


#####################################################################################################################################
#FUNCTION NO2: BINOMIAL 
#Size=number of cells
#Number of successes : from 0 to as many cells in the well*dropout
#Probability= 1/total number of samples in the pool

generate_distribution <- function(all_samples, samples_per_pool, well, outfile) {
  pools = all_samples / samples_per_pool
  dropout<-c(1/2)
  ...
  #plot_cells(samples_per_pool, well, counts, outfile)
